...
// ─── MAGNETIC BUTTON ──────────────────────────────────────────────────────────
function MagneticButton({ children, className, href = '#', onClick, to }) {
  const ref = useRef(null);
      ...
      {[0, 1].map(i => (
        ...
        >
          {items.map((item, j) => (
            <span
              key={j}
              className="flex items-center gap-4 text-[11px] font-bold text-white/25 uppercase tracking-[0.2em]"
...
// ─── SHARED LAYOUT (Navbar + Footer + Global WhatsApp) ────────────────────────
function SharedLayout() {
  ...
  return (
    <div dir={isArabic ? 'rtl' : 'ltr'} className="min-h-screen bg-[#F7F5F1] text-[#0D1B2A]" style={{ fontFamily: FONT_BODY }}>
      <NoiseOverlay />
      ...
      >
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-4 flex items-center justify-between">
          <Link to="/">
            <img src={logo} alt="AIOON" className={`h-8 md:h-9 w-auto transition-all duration-300 ${scrollY > 60 ? '' : 'brightness-[10]'}`} />
          </Link>
          ...
          <ul className="hidden md:flex items-center gap-8 text-sm font-medium" style={{ fontFamily: FONT_NAV }}>
            {[
              ...
              <li key={to} className={`relative group cursor-pointer transition-colors duration-300 ${scrollY > 60 ? 'text-[#0D1B2A]/70 hover:text-[#0D1B2A]' : 'text-white/80 hover:text-white'}`}>
            ...
            <li className="relative">
              <button
                ...
                className={`text-sm font-semibold transition-colors duration-300 ${scrollY > 60 ? 'text-[#0D1B2A]/60 hover:text-[#0D1B2A]' : 'text-white/60 hover:text-white'}`}
              ...
              <AnimatePresence>
                {showLangDropdown && (
                  <motion.div
                    ...
                    className="absolute top-full right-0 mt-3 w-32 bg-white rounded-xl shadow-2xl border border-black/5 overflow-hidden"
                  >
                    {[['en', 'English'], ['ar', '\u0627\u0644\u0639\u0631\u0628\u064a\u0629']].map(([code, label]) => (
                      <button
                        key={code}
                        onClick={() => { setIsArabic(code === 'ar'); setShowLangDropdown(false); }}
                        className={`w-full px-4 py-2.5 text-left text-sm transition-colors ${(code === 'en') === !isArabic ? 'bg-red-50 text-red-600 font-semibold' : 'hover:bg-gray-50 text-gray-700'}`}
                        style={{ fontFamily: FONT_NAV }}
                      >
                        {label}
                      </button>
          ...
          <MagneticButton
            to="/"
            className={`hidden md:inline-flex items-center gap-2 font-semibold px-5 py-2.5 rounded-full text-sm transition-all duration-400 ${scrollY > 60 ? 'bg-[#0D1B2A] text-white hover:bg-red-600 shadow-md' : 'bg-white/15 text-white border border-white/30 hover:bg-white hover:text-[#0D1B2A] backdrop-blur-sm'}`}
            style={{ fontFamily: FONT_NAV }}
          >
          ...
          {/* Mobile hamburger */}
          <button
            className={`md:hidden text-xl transition-colors ${scrollY > 60 ? 'text-[#0D1B2A]' : 'text-white'}`}
              ...
              <div className="px-5 py-5 flex flex-col gap-4">
                ...
                <div className="flex gap-2 pt-1">
                  {[['en', 'English'], ['ar', '\u0627\u0644\u0639\u0631\u0628\u064a\u0629']].map(([code, label]) => (
                    <button
                      key={code}
                      onClick={() => { setIsArabic(code === 'ar'); setIsMenuOpen(false); }}
                      className={`flex-1 py-2 rounded-lg text-xs font-semibold transition-colors ${(code === 'en') === !isArabic ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700'}`}
                      style={{ fontFamily: FONT_NAV }}
                    >
                      {label}
                    </button>
                ...
                <Link to="/" className="bg-[#0D1B2A] text-white text-sm font-semibold px-5 py-3 rounded-full text-center" style={{ fontFamily: FONT_NAV }} onClick={() => setIsMenuOpen(false)}>
                  Book a call &#8599;
                </Link>
              </div>
            </motion.div>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10 mb-12">
            <div className="md:col-span-2">
              ...
              <p className="text-gray-600 text-[12px] leading-relaxed max-w-xs" style={{ fontFamily: FONT_BODY }}>
                Pioneering digital transformation in Saudi Arabia with innovative technology solutions aligned with Vision 2030.
              </p>
              <div className="flex gap-2.5 mt-6">
                {['in', 'tw', 'fb', 'ig'].map((s, i) => (
                  <motion.a key={i} href="#" whileHover={{ y: -3, scale: 1.1 }} className="w-8 h-8 rounded-lg bg-white/5 hover:bg-red-600 flex items-center justify-center text-gray-500 hover:text-white text-[11px] font-bold uppercase transition-colors duration-300" style={{ fontFamily: FONT_BODY }}>{s}</motion.a>
                ))}
              </div>
            </div>
            <div>
              ...
              <ul className="space-y-2.5">
                {[
                  ...
                  <li key={to}>
                    <Link to={to} className="text-gray-600 hover:text-red-400 text-[12px] transition-colors duration-300 flex items-center gap-2 group" style={{ fontFamily: FONT_NAV }}>
                      <span className="w-0 group-hover:w-2.5 h-px bg-red-500 transition-all duration-300" />{label}
                    </Link>
                  </li>
                ))}
                <li><a href="/#services-section" className="text-gray-600 hover:text-red-400 text-[12px] transition-colors duration-300 flex items-center gap-2 group" style={{ fontFamily: FONT_NAV }}><span className="w-0 group-hover:w-2.5 h-px bg-red-500 transition-all duration-300" />{t.services}</a></li>
                <li><a href="/#contact-section"  className="text-gray-600 hover:text-red-400 text-[12px] transition-colors duration-300 flex items-center gap-2 group" style={{ fontFamily: FONT_NAV }}><span className="w-0 group-hover:w-2.5 h-px bg-red-500 transition-all duration-300" />{t.contact}</a></li>
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-2.5">
                {['Riyadh, Saudi Arabia', 'info@aioon.com', '+966 535 141 447'].map((item, i) => (
                  <li key={i} className="text-gray-600 hover:text-gray-400 text-[12px] transition-colors duration-300 cursor-pointer" style={{ fontFamily: FONT_BODY }}>{item}</li>
                ))}
              </ul>
            </div>
          </div>
          <div className="border-t border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-gray-700 text-[11px]" style={{ fontFamily: FONT_BODY }}>&copy; {new Date().getFullYear()} AIOON Technologies. All rights reserved.</p>
            <p className="text-gray-700 text-[11px]" style={{ fontFamily: FONT_BODY }}>Proudly supporting Saudi Vision 2030 &#x1F1F8;&#x1F1E6;</p>
          </div>
        </div>
      </footer>

      {/* ── GLOBAL WHATSAPP BUTTON ── shown on ALL pages via SharedLayout ── */}
      <motion.button
        ...
        {showWhatsApp && (
          <>
            ...
            >
              ...
              <button onClick={() => setShowWhatsApp(false)} className="absolute top-2 right-2 text-white/60 hover:text-white bg-black/15 rounded-full w-4 h-4 flex items-center justify-center text-[10px] font-bold transition-colors">&times;</button>
            </motion.div>
...
function HomePage() {
  ...
  // ── WHO WE ARE stats (renamed from aboutStats) ──
  const whoWeAreStats = [
  ...
  return (
    <div ref={mainRef}>
            ...
            <h1 className="font-black leading-[1.04] mb-4 text-white">
              {t.heroTitle.map((line, i) => (
                <div key={i} className="overflow-hidden">
                  <span className={`hero-line block text-[clamp(2.2rem,5.2vw,4rem)] ${i === 1 ? 'text-red-400' : ''}`} style={{ fontFamily: FONT_HERO }}>{line}</span>
                </div>
              ))}
            </h1>
            <p className="hero-desc text-white/55 text-sm md:text-[14px] leading-relaxed mb-6 max-w-md font-light" style={{ fontFamily: FONT_BODY }}>{t.heroDesc}</p>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
          <motion.div className="mb-7" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <div className="flex items-center gap-3 mb-2">
              ...
              {/* ── WHO WE ARE (renamed from "About Aioon Technologies") ── */}
              <span className="text-red-600 text-[10px] font-bold tracking-[0.3em] uppercase" style={{ fontFamily: FONT_BADGE }}>Who We Are</span>
            </div>
          ...
          <div className="grid md:grid-cols-3 gap-0 md:divide-x md:divide-[#0D1B2A]/8 mb-8">
            {[
              ...
              <motion.div key={i} className={`pb-6 md:pb-0 ${i > 0 ? 'md:px-8 pt-6 md:pt-0' : 'md:pr-8'}`} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.12 }}>
                ...
                {i === 2 && (
                  <div className="mt-4">
                    <MagneticButton to="/about" className="inline-flex items-center gap-2 border border-[#0D1B2A]/15 hover:border-red-500/60 text-[#0D1B2A]/60 hover:text-red-600 font-semibold px-5 py-2 rounded-full text-xs transition-all duration-300" style={{ fontFamily: FONT_NAV }}>
          ...
          <div className="grid md:grid-cols-[1fr_340px] gap-5 items-stretch">
            <div className="relative rounded-2xl overflow-hidden" style={{ height: '340px' }} onMouseEnter={() => setElvDragging(true)} onMouseLeave={() => setElvDragging(false)}>
              <AnimatePresence mode="wait">
                <motion.div key={`img-${currentElv.id}`} className="absolute inset-0" initial={{ opacity: 0, scale: 1.04 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-1.5 justify-center pt-1">
                {elvProducts.map((_, i) => (<button key={i} onClick={() => { setActiveElvIndex(i); setElvDragging(true); setTimeout(() => setElvDragging(false), 4000); }} className={`rounded-full transition-all duration-400 ${i === activeElvIndex ? 'w-5 h-1.5 bg-red-500' : 'w-1.5 h-1.5 bg-white/15 hover:bg-white/30'}`} />))}
      ...
      {/* ── CONTACT ── */}
      <section id="contact-section" className="py-10 md:py-14 relative overflow-hidden bg-[#F7F5F1]">
        <div className="absolute inset-0 opacity-[0.035]" style={{ backgroundImage: 'radial-gradient(circle, #0D1B2A 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
          ...
          <div className="grid md:grid-cols-[1fr_1.6fr] gap-10 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              ...
              <h2 className="text-2xl md:text-3xl font-black text-[#0D1B2A] leading-tight mb-3" style={{ fontFamily: FONT_HEADING }}>
                Ready for Digital<br /><span className="text-red-600">Transformation?</span>
              </h2>
              <p className="text-[#0D1B2A]/50 text-[11px] leading-relaxed mb-5 max-w-xs" style={{ fontFamily: FONT_BODY }}>Our team is ready to guide you from discovery to deployment.</p>
            ...
            <motion.div
              className="grid grid-cols-3 divide-x divide-[#0D1B2A]/8 border border-[#0D1B2A]/8 rounded-2xl overflow-hidden bg-white/60 backdrop-blur-sm shadow-sm"
